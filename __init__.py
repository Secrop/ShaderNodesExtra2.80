    ...
    for ident in nc:
        for idx, category in enumerate(nc[ident][0]):
            if category.identifier==catid and category.name==catname:
                return ident, category, nc[ident][2][idx], nc[ident][3][idx]
...
def addCat(category, ident='SHADER'):
